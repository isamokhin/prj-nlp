...
PICKLE = "uk_perceptron_tagger.pickle"
...
class PerceptronTagger(BaseTagger):
    ...
    def tag(self, corpus, tokenize=True):
        ...
        s_split = tokenize_sents if tokenize else lambda t: t.split('\n')
        w_split = tokenize_words if tokenize else lambda s: s.split()
        ...
        for word, tag in tokens:
            if self.is_unambigous(word) and tag != 'PROPN':
                res_tokens.append((word, self.is_unambigous(word)))
            else:
                res_tokens.append((word, tag))
        return res_tokens
    ...
    def _get_features(self, i, word, context, prev, prev2):
        ...
        add('bias')
        add('is_punctuation', str(word in string.punctuation))
        add('i suffix', word[-3:])
        add('i pref1', word[0])
        add('i-1 tag', prev)
        add('i-2 tag', prev2)
